...
if [ -d "/data/data/com.termux" ]; then
    ENV="termux"
    WORK_DIR="$HOME/tool_projects/site-diagnostic"
else
    ENV="ubuntu"
    WORK_DIR="$HOME/site-diagnostic"
fi
...
# Générer un ID de session sans pwgen
SESSION_ID="$(cat /dev/urandom | tr -dc 'a-z' | head -c 6)"
LOG_FILE="$WORK_DIR/session_$(date +"%Y-%m-%d_%H-%M-%S")_$SESSION_ID.log"
touch "$LOG_FILE"
...
check_dns() {
    ...
    if command -v dig &> /dev/null; then
        ...
        echo -e "\033[33m[INFO]\033[0m 'dig' non installe."
        if [ "$ENV" = "termux" ]; then
            echo "    Installe avec : pkg install dnsutils"
        else
            echo "    Installe avec : sudo apt install dnsutils"
        fi
    fi
    echo ""
}
...
check_traceroute() {
    ...
    if command -v traceroute &> /dev/null; then
        ...
        echo -e "\033[33m[INFO]\033[0m 'traceroute' non installe."
        if [ "$ENV" = "termux" ]; then
            echo "    Installe avec : pkg install traceroute"
        else
            echo "    Installe avec : sudo apt install traceroute"
        fi
    fi
    echo ""
}
...
check_whois() {
    ...
    if command -v whois &> /dev/null; then
        ...
        echo -e "\033[33m[INFO]\033[0m 'whois' non installe."
        if [ "$ENV" = "termux" ]; then
            echo "    Installe avec : pkg install whois"
        else
            echo "    Installe avec : sudo apt install whois"
        fi
    fi
    echo ""
}
...
save_html() {
    ...
    HTML_FILE="${WORK_DIR}/${CLEAN_SITE}_${TIMESTAMP}_$SESSION_ID.html"
